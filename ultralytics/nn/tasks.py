    ...
    def fuse(self, verbose=True):
        ...
        if not self.is_fused():
            for m in self.model.modules():
                if isinstance(m, (Conv, Conv2, DWConv, LightConv)) and hasattr(m, "bn"):
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2fCIB, RGCSPELAN, RGELAN,
                     C2f_AKConv, RepBlock, VoVGSCSPns, C2fGhostCIB, C2fGhostCIB_L, C2fGhost, RGELANnoRep):
            ...
            args = [ch[f], *args]
        elif m in {DySample, FocalModulation}:
            c2 = ch[f]
            args = [c2, *args]
